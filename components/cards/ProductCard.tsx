...
const ProductCard = ({stripeProductId, name, description, stock, price, category, photo} : ProductType) => {
  ...
  return (
    <div key={stripeProductId} className="relative group bg-gray-100 p-4 xl:p-4 max-sm:px-8 rounded-xl hover:bg-black hover:text-white">
    <Link className="absolute inset-0" href={`/products/${stripeProductId}`}>
